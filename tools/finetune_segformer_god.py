"""
finetune_segformer_god.py

Fine-tunes AVMI 8-class SegFormer-B2 on GOD (GOOSE Offroad Dataset).
  sky | tree | bush | ground | obstacle | rock | water | concrete

GOD annotation: pylon_camera_node_label_id/*.png
  - Stores ontology IDs directly (verified against color annotations)
  - No off-by-one shift (unlike RUGD _orig.png)

Single sequence 00000 with 1199 images → 80% train / 20% val split.

Starts from:  work_dirs/segformer_b2_avmi_8class/latest.pth
Saves to:     work_dirs/segformer_b2_finetune_god/

Run on Turing HPC via train_finetune_god.sh
"""
...
WORK_DIR     = os.path.join(_REPO_ROOT, 'work_dirs', 'segformer_b2_finetune_god')
...
WARMUP_ITERS = 500
CROP_SIZE    = 512
VAL_SPLIT    = 0.2   # last 20% of sorted files used for val

CLASSES = ('sky', 'tree', 'bush', 'ground', 'obstacle', 'rock', 'water', 'concrete')
# Higher weight for rare classes and new classes absent from AVMI training
CLASS_WEIGHTS = torch.tensor([0.5, 1.0, 1.5, 0.8, 2.5, 3.0, 2.5, 2.5])

# ── Class mapping ─────────────────────────────────────────────────────────────
# GOD ontology IDs stored directly in label_id PNGs (verified):
...
class GODDataset(Dataset):
    def __init__(self, split='train', train_aug=True):
        ...
        ann_root = Path(GOD_ROOT) / 'pylon_camera_node_label_id'

        # Collect all files across sequences, sorted for deterministic split
        all_pairs = []
        for seq in sorted(img_root.iterdir()):
            for f in sorted(seq.glob('*.png')):
                ann_path = ann_root / seq.name / f.name
                if ann_path.exists():
                    all_pairs.append((str(f), str(ann_path)))

        # 80/20 split on sorted list
...
def main():
    ...
    train_ds = GODDataset('train', train_aug=True)
    val_ds   = GODDataset('val',   train_aug=False)
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True,
                              num_workers=4, pin_memory=True, drop_last=True)
    val_loader   = DataLoader(val_ds,   batch_size=4, shuffle=False, num_workers=0)
    print(f'Train: {len(train_ds)} | Val: {len(val_ds)} GOD images')
