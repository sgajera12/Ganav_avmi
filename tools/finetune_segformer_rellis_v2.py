"""
finetune_segformer_rellis_v2.py

Fine-tunes AVMI 8-class SegFormer-B2 on RELLIS with clean 8-class mapping:
  sky | tree | bush | ground | obstacle | rock | water | concrete
  - water(6) + puddle(29)  → water(6)
  - concrete(23)           → concrete(7)
  - mud(31)                → ignore(255)
  - rubble(33)             → rock(5)

Starts from:  work_dirs/segformer_b2_avmi_8class/latest.pth
Saves to:     work_dirs/segformer_b2_finetune_rellis_v2/
"""
...
WORK_DIR     = os.path.join(_REPO_ROOT, 'work_dirs', 'segformer_b2_finetune_rellis_v2')
...
# Higher weight for new classes water/concrete since they were absent from AVMI training
...
_RELLIS_MAP[6]  = 6  # water     → water (class 6)
...
_RELLIS_MAP[23] = 7  # concrete  → concrete (class 7)
_RELLIS_MAP[27] = 4  # barrier   → obstacle
_RELLIS_MAP[29] = 6  # puddle    → water (class 6)
_RELLIS_MAP[31] = 255 # mud      → IGNORE (ambiguous)
...
def main():
    ...
    train_ds = RELLISDataset('train', train_aug=True)
    val_ds   = RELLISDataset('val',   train_aug=False)

    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True,
                              num_workers=4, pin_memory=True, drop_last=True)
    val_loader   = DataLoader(val_ds,   batch_size=4, shuffle=False, num_workers=0)
    print(f'Train: {len(train_ds)} RELLIS images')
    print(f'Val:   {len(val_ds)}   RELLIS images')
