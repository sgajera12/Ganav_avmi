"""
finetune_segformer_rugd_rellis.py

Fine-tunes our AVMI-trained SegFormer-B2 (80.51% mIoU) on RUGD + RELLIS
with 6-class mapping:  sky | tree | bush | ground | obstacle | rock

Starts from:  work_dirs/segformer_b2_avmi_hf/latest.pth
Saves to:     work_dirs/segformer_b2_finetune_rugd_rellis/

Run on Turing HPC via train_finetune.sh
"""
...
WORK_DIR     = os.path.join(_REPO_ROOT, 'work_dirs', 'segformer_b2_finetune_rugd_rellis')
...
def main():
    ...
    rugd_train   = RUGDDataset('train',  train_aug=True)
    rellis_train = RELLISDataset('train', train_aug=True)
    train_ds     = ConcatDataset([rugd_train, rellis_train])

    rugd_val     = RUGDDataset('val',   train_aug=False)
    rellis_val   = RELLISDataset('val',  train_aug=False)
    val_ds       = ConcatDataset([rugd_val, rellis_val])

    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True,
                              num_workers=2, pin_memory=True, drop_last=True)
    val_loader   = DataLoader(val_ds,   batch_size=4, shuffle=False, num_workers=0)
    print(f'Train: {len(train_ds)} (RUGD {len(rugd_train)} + RELLIS {len(rellis_train)})')
    print(f'Val:   {len(val_ds)}   (RUGD {len(rugd_val)}   + RELLIS {len(rellis_val)})')
    ...
    for itr in range(start_iter, MAX_ITERS):
        ...
        if itr % 50 == 0:
            ...
            msg = (f'[{itr:6d}/{MAX_ITERS}] loss={loss.item():.4f} '
