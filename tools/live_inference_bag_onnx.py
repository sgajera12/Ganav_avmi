"""
Live inference from ROS2 bag using ONNX Runtime GPU.
No mmcv, no PyTorch version constraints — runs on RTX 5070 natively.

Prerequisites:
    1. Export model first (in ganav env):
       conda run -n ganav python tools/export_onnx.py

    2. Install onnxruntime-gpu in segformer env:
       /home/pinaka/miniconda3/envs/segformer/bin/pip install onnxruntime-gpu

    3. Run this script (in segformer env):
       cd ~ && /home/pinaka/miniconda3/envs/segformer/bin/python \
           /home/pinaka/GANav-offroad/tools/live_inference_bag_onnx.py

Controls:
  SPACE  — pause / resume
  D / →  — step one frame forward
  A / ←  — step one frame back
  + / =  — speed up (2x, 4x, …)
  -      — slow down (0.5x, 0.25x, …)
  1      — reset to real-time (1x)
  Q/ESC  — quit
"""

import os, sys, struct, sqlite3, time
import numpy as np
import cv2
import onnxruntime as ort

# ── Config ────────────────────────────────────────────────────────────────────
BAG_PATH   = '/home/pinaka/dataset/AVMI/dataset1/dataset1_0.db3'
CAM_TOPIC  = '/camera/right/image_raw'
LID_TOPIC  = '/lidar/points2'
ONNX_PATH  = '/home/pinaka/GANav-offroad/work_dirs/ganav_avmi_scratch/model.onnx'
IMG_H, IMG_W = 300, 375   # GANav model input size (detected via backbone hook during export)
DISPLAY_W  = 640
SKIP       = 1   # process every Nth frame

# ── Camera intrinsics (Unreal Engine calibration) ─────────────────────────────
_K = np.array([[278.5899, 0, 323.7999],
               [0, 334.2900, 229.1999],
               [0, 0, 1]], dtype=np.float32)

# ── LiDAR→Camera extrinsics (pre-computed from generate_pointcloud_grids.py) ──
# R_L2C and t_L2C computed from UE positions + -2° pitch correction
_R_L2C = np.array([[ 0.0,      -1.0,      0.0     ],
                   [-0.034899,  0.0,      -0.999391],
                   [ 0.999391,  0.0,      -0.034899]], dtype=np.float32)
_t_L2C = np.array([-0.15, 0.07989, 0.86414], dtype=np.float32)

# ImageNet normalization (same as GANav preprocessing)
MEAN = np.array([123.675, 116.28,  103.53],  dtype=np.float32)
STD  = np.array([ 58.395,  57.12,   57.375], dtype=np.float32)

# ── Class palette (BGR for OpenCV) ────────────────────────────────────────────
PALETTE_BGR = np.array([
    [178, 102,  24],  # 0 sky
    [ 37, 182,  18],  # 1 tree
    [ 15, 255, 239],  # 2 bush
    [  6,  19,  92],  # 3 ground
    [250,  63, 255],  # 4 obstacle
    [  0,   0, 255],  # 5 rock
], dtype=np.uint8)

CLASS_NAMES = ['sky', 'tree', 'bush', 'ground', 'obstacle', 'rock']

# ── CDR PointCloud2 parser ────────────────────────────────────────────────────
def _read_str(data, offset):
    n = struct.unpack_from('<I', data, offset)[0]; offset += 4
    s = data[offset: offset + n - 1].decode(errors='ignore'); offset += n
    if offset % 4: offset += 4 - offset % 4
    return s, offset

def parse_pointcloud2(data: bytes):
    """CDR PointCloud2 → Nx3 float32 xyz."""
    offset = 4 + 8  # CDR header + stamp
    _, offset = _read_str(data, offset)          # frame_id
    offset += 8                                  # height + width
    nfields = struct.unpack_from('<I', data, offset)[0]; offset += 4
    for _ in range(nfields):
        _, offset = _read_str(data, offset)
        offset += 9
        if offset % 4: offset += 4 - offset % 4
    offset += 1                                  # is_bigendian
    if offset % 4: offset += 4 - offset % 4
    point_step = struct.unpack_from('<I', data, offset)[0]; offset += 4
    offset += 4                                  # row_step
    data_len = struct.unpack_from('<I', data, offset)[0]; offset += 4
    pts = np.frombuffer(data[offset: offset + data_len],
                        dtype=np.float32).reshape(-1, point_step // 4)
    return pts[:, :3]                            # x, y, z only

def project_lidar(xyz, img_h, img_w):
    """Project Nx3 LiDAR points → (u, v) pixel coords (in-frustum only)."""
    Pc = (_R_L2C @ xyz.T + _t_L2C.reshape(3, 1)).T
    Z  = Pc[:, 2]
    mask = Z > 0.1
    Pc, Z = Pc[mask], Z[mask]
    uv = (_K @ (Pc.T / Z)).T
    u, v = uv[:, 0], uv[:, 1]
    inside = (u >= 0) & (u < img_w) & (v >= 0) & (v < img_h)
    return u[inside].astype(np.int32), v[inside].astype(np.int32)

def draw_dots(canvas, u, v, colors):
    """Draw colored circles at (u,v) on canvas in-place."""
    for x, y, c in zip(u, v, colors):
        cv2.circle(canvas, (int(x), int(y)), 2,
                   (int(c[0]), int(c[1]), int(c[2])), -1)

# ── CDR image parser ──────────────────────────────────────────────────────────
def parse_image(data: bytes):
    offset = 4
    offset += 8
    fid_len = struct.unpack_from('<I', data, offset)[0]; offset += 4
    offset += fid_len
    if offset % 4: offset += 4 - offset % 4
    height = struct.unpack_from('<I', data, offset)[0]; offset += 4
    width  = struct.unpack_from('<I', data, offset)[0]; offset += 4
    enc_len = struct.unpack_from('<I', data, offset)[0]; offset += 4
    enc = data[offset: offset + enc_len - 1].decode(errors='ignore')
    offset += enc_len
    offset += 1
    if offset % 4: offset += 4 - offset % 4
    offset += 4
    data_len = struct.unpack_from('<I', data, offset)[0]; offset += 4
    raw = np.frombuffer(data[offset: offset + data_len], dtype=np.uint8)
    if enc in ('rgb8', 'RGB8'):
        img = raw.reshape(height, width, 3)[..., ::-1].copy()
    elif enc in ('bgr8', 'BGR8'):
        img = raw.reshape(height, width, 3).copy()
    else:
        img = raw.reshape(height, width, -1).copy()
    return img

# ── Preprocessing ─────────────────────────────────────────────────────────────
def preprocess(img_bgr):
    """BGR uint8 → NCHW float32 normalized tensor for ONNX."""
    img = cv2.resize(img_bgr, (IMG_W, IMG_H))
    img = img[:, :, ::-1].astype(np.float32)   # BGR→RGB
    img = (img - MEAN) / STD                    # normalize
    img = img.transpose(2, 0, 1)               # HWC→CHW
    img = img[np.newaxis, ...]                  # add batch dim
    return np.ascontiguousarray(img)

# ── Postprocessing ────────────────────────────────────────────────────────────
def seg_to_color(logits, orig_h, orig_w):
    """(1,6,H,W) logits → BGR colour mask resized to original size."""
    seg = np.argmax(logits[0], axis=0).astype(np.uint8)  # (H,W)
    seg = np.clip(seg, 0, 5)
    colour = PALETTE_BGR[seg.reshape(-1)].reshape(seg.shape[0], seg.shape[1], 3)
    return cv2.resize(colour, (orig_w, orig_h), interpolation=cv2.INTER_NEAREST)

def draw_legend(img):
    H, W = img.shape[:2]
    cell_h = 22
    legend_h = len(CLASS_NAMES) * cell_h + 6
    overlay = img.copy()
    cv2.rectangle(overlay, (0, H - legend_h), (160, H), (20, 20, 20), -1)
    cv2.addWeighted(overlay, 0.6, img, 0.4, 0, img)
    for i, name in enumerate(CLASS_NAMES):
        y = H - legend_h + 4 + i * cell_h
        col = tuple(int(c) for c in PALETTE_BGR[i])
        cv2.rectangle(img, (4, y), (18, y + cell_h - 4), col, -1)
        cv2.putText(img, name, (22, y + cell_h - 7),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (230, 230, 230), 1, cv2.LINE_AA)
    return img

# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    # Load ONNX model with GPU
    if not os.path.exists(ONNX_PATH):
        print(f'ERROR: ONNX model not found at {ONNX_PATH}')
        print('Run first: conda run -n ganav python tools/export_onnx.py')
        sys.exit(1)

    print('Loading ONNX model on GPU...')
    providers = ['CUDAExecutionProvider', 'CPUExecutionProvider']
    sess = ort.InferenceSession(ONNX_PATH, providers=providers)
    active = sess.get_providers()[0]
    print(f'Running on: {active}')

    # Open bag
    conn = sqlite3.connect(BAG_PATH)
    rows = conn.execute("SELECT id, name FROM topics").fetchall()
    topic_map = {name: tid for tid, name in rows}

    if CAM_TOPIC not in topic_map:
        print(f'ERROR: topic {CAM_TOPIC} not in bag. Available: {list(topic_map.keys())}')
        sys.exit(1)

    frames = conn.execute(
        "SELECT timestamp, data FROM messages WHERE topic_id=? ORDER BY timestamp",
        (topic_map[CAM_TOPIC],)
    ).fetchall()
    frames = frames[::SKIP]
    total  = len(frames)

    # Load LiDAR for point-cloud panels (optional)
    has_lidar = LID_TOPIC in topic_map
    if has_lidar:
        lid_rows = conn.execute(
            "SELECT timestamp, data FROM messages WHERE topic_id=? ORDER BY timestamp",
            (topic_map[LID_TOPIC],)
        ).fetchall()
        lid_ts   = np.array([r[0] for r in lid_rows], dtype=np.int64)
        lid_data = [bytes(r[1]) for r in lid_rows]
        print(f'LiDAR frames: {len(lid_rows)}')
    else:
        print(f'WARNING: {LID_TOPIC} not in bag — point-cloud panels will be blank')

    print(f'{total} camera frames loaded. Starting inference\n')

    # Display
    win = 'GANav ONNX GPU | SPACE=pause  D=fwd  A=back  +/-=speed  1=realtime  Q=quit'
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)

    paused = False
    speed  = 1.0   # playback multiplier (1.0 = real-time, 2.0 = 2x, 0.5 = half)
    idx    = 0

    while 0 <= idx < total:
        t_frame_start = time.monotonic()
        ts, raw = frames[idx]

        try:
            img_bgr = parse_image(bytes(raw))
        except Exception as e:
            print(f'Frame {idx}: parse error — {e}')
            idx += 1
            continue

        orig_h, orig_w = img_bgr.shape[:2]

        # Inference
        inp    = preprocess(img_bgr)
        logits = sess.run(None, {'image': inp})[0]  # (1,6,IMG_H,IMG_W)

        # seg_map: class index at original resolution (needed for point coloring)
        seg_map_small = np.argmax(logits[0], axis=0).astype(np.uint8)
        seg_map = cv2.resize(seg_map_small, (orig_w, orig_h),
                             interpolation=cv2.INTER_NEAREST)
        colour_mask = PALETTE_BGR[seg_map.reshape(-1)].reshape(orig_h, orig_w, 3)
        seg_bgr = draw_legend(colour_mask.copy())

        # ── Point cloud panels ────────────────────────────────────────────────
        scale = DISPLAY_W / orig_w
        dH    = int(orig_h * scale)

        if has_lidar:
            li = int(np.argmin(np.abs(lid_ts - ts)))
            try:
                xyz = parse_pointcloud2(lid_data[li])
                xyz = xyz[np.all(np.isfinite(xyz), axis=1)]
                u, v = project_lidar(xyz, orig_h, orig_w)

                # raw: white dots on camera image background
                pc_raw = img_bgr.copy()
                draw_dots(pc_raw, u, v,
                          np.full((len(u), 3), 220, dtype=np.uint8))

                # segmented: class-coloured dots on black
                pc_seg = np.zeros((orig_h, orig_w, 3), dtype=np.uint8)
                draw_dots(pc_seg, u, v, PALETTE_BGR[np.clip(seg_map[v, u], 0, 5)])
            except Exception as e:
                print(f'Frame {idx}: lidar error — {e}')
                pc_raw = np.zeros((orig_h, orig_w, 3), dtype=np.uint8)
                pc_seg = np.zeros((orig_h, orig_w, 3), dtype=np.uint8)
        else:
            pc_raw = np.zeros((orig_h, orig_w, 3), dtype=np.uint8)
            pc_seg = np.zeros((orig_h, orig_w, 3), dtype=np.uint8)

        # ── Build 2-row display ───────────────────────────────────────────────
        vdiv = np.full((dH, 4, 3), 60, dtype=np.uint8)

        left  = cv2.resize(img_bgr, (DISPLAY_W, dH))
        right = cv2.resize(seg_bgr, (DISPLAY_W, dH), interpolation=cv2.INTER_NEAREST)
        bl    = cv2.resize(pc_raw,  (DISPLAY_W, dH), interpolation=cv2.INTER_NEAREST)
        br    = cv2.resize(pc_seg,  (DISPLAY_W, dH), interpolation=cv2.INTER_NEAREST)

        cv2.putText(left,  'Original',     (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
        cv2.putText(right, 'Segmentation', (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
        cv2.putText(bl,    'Raw Points',   (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
        cv2.putText(br,    'Seg. Points',  (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)

        speed_str = f'{speed:.2g}x'
        info = f'Frame {idx+1}/{total}  ts={ts/1e9:.2f}s  {speed_str}  {"[PAUSED]" if paused else ""}'
        cv2.putText(left, info, (10, dH-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1)

        hdiv = np.full((4, DISPLAY_W * 2 + 4, 3), 60, dtype=np.uint8)
        top_row    = np.hstack([left, vdiv, right])
        bottom_row = np.hstack([bl,   vdiv, br])
        cv2.imshow(win, np.vstack([top_row, hdiv, bottom_row]))

        # Compute delay based on timestamp gap to next frame, scaled by speed
        if not paused and idx + 1 < total:
            next_ts = frames[idx + 1][0]
            real_gap_s = (next_ts - ts) / 1e9          # seconds between frames in bag
            target_gap_s = real_gap_s / speed
            elapsed = time.monotonic() - t_frame_start
            wait_ms = max(1, int((target_gap_s - elapsed) * 1000))
        else:
            wait_ms = 0  # paused: block until keypress

        key = cv2.waitKey(wait_ms if not paused else 0) & 0xFF
        if key in (ord('q'), 27):
            break
        elif key == ord(' '):
            paused = not paused
        elif key in (83, ord('d')):
            idx = min(idx + 1, total - 1); continue
        elif key in (81, ord('a')):
            idx = max(0, idx - 1); paused = True; continue
        elif key in (ord('+'), ord('=')):
            speed = min(speed * 2, 32.0)
        elif key == ord('-'):
            speed = max(speed / 2, 0.125)
        elif key == ord('1'):
            speed = 1.0

        if not paused:
            idx += 1

    cv2.destroyAllWindows()
    conn.close()
    print('Done.')

if __name__ == '__main__':
    main()
