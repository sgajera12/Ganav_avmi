"""
SegFormer-B2 training on AVMI dataset with 8-class output head.
Classes: sky | tree | bush | ground | obstacle | rock | water | concrete
AVMI labels only cover classes 0-5; classes 6-7 have zero AVMI supervision
(partial-label setup — fine-tuned later on RELLIS/RUGD to learn water/concrete).

Run on Turing HPC via train_avmi_8class.sh
"""
...
def rgb_mask_to_index(rgb_img):
    ...
    h, w = rgb_img.shape[:2]#writes teh shapes of image to h and w 
    flat = rgb_img.reshape(-1, 3).astype(np.int32)#make image to array by converting each pixel to a each row of 3gb values 
...
WORK_DIR    = os.path.join(_REPO_ROOT, 'work_dirs', 'segformer_b2_avmi_8class')
...
CLASSES = ('sky', 'tree', 'bush', 'ground', 'obstacle', 'rock', 'water', 'concrete')
# Classes 6 (water) and 7 (concrete) have no AVMI labels; low weight keeps them dormant
CLASS_WEIGHTS = torch.tensor([0.5, 1.0, 1.5, 0.8, 2.5, 3.0, 0.5, 0.5])
